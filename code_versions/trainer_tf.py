...
# Load a sample audio
sample_audio_path = 'preprocessed_data/test/mixture/3_000.wav'
sample_audio, _ = librosa.load(sample_audio_path, sr=SAMPLE_RATE)
# Compute the Mel spectrogram
sample_stft = librosa.stft(sample_audio)
sample_phase = np.angle(sample_stft)

# Compute the Mel spectrogram
sample_mel = librosa.feature.melspectrogram(sample_audio, sr=SAMPLE_RATE, n_mels=128)
sample_mel = pad_mel_spectrogram(sample_mel)
sample_mel = tf.expand_dims(sample_mel, axis=-1)

# Make the prediction
predictions = model.predict(tf.expand_dims(sample_mel, axis=0))

# Print the predictions
print('Prediction:', predictions)

# Invert the Mel spectrogram
def invert_mel_spectrogram(mel_spectrogram, phase):
    # Pad the phase to match the size of mel_spectrogram
    pad_width = mel_spectrogram.shape[1] - phase.shape[1]
    if pad_width > 0:
        phase = np.pad(phase, ((0, 0), (0, pad_width)), mode='constant')

    # Apply inverse Mel transform
    mel_inverse = librosa.feature.inverse.mel_to_stft(mel_spectrogram, sr=SAMPLE_RATE)
    # Apply inverse STFT
    audio_inverse = librosa.istft(mel_inverse * np.exp(1j * phase))
    return audio_inverse

# Invert the Mel spectrogram for each source
for i in range(num_sources):
    source_mel = predictions[0, :, :, i]
    source_phase = sample_phase
    source_audio = invert_mel_spectrogram(source_mel, source_phase)
    output_path = f'outputs/{i+1}.wav'
    librosa.output.write_wav(output_path, source_audio, SAMPLE_RATE)
    print(f'Source {i} audio saved at:', output_path)
'''